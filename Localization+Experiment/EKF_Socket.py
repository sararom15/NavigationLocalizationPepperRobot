...
what the code does:
 Estimation of the Robot state and send this information via Socket to the node that will be responsable of the motion.
"""
...
# -- DEFINE INITIAL POSITION OF PEPPER IN THE WORLD FRAME --
init_position = np.array([[0.0], [0.0], [-1.57]])
...
# -- Define GROUND TRUTH in order to Plot the error of the estimation of the state by EKF -- #
GROUNDTRUTH = np.array([[0.3], [-0.7], [0.60]])
...
R_t = np.array([[0.8, 0, 0], [0, 0.8, 0], [0, 0, 0.8]])
...
def EKF(session, realMeasurement, x, p, deltat):
    """
    perform state estimation
    INPUT:
    -'session' for accessing the velocities values;
    -'realMeasurement': queue
    -'x': previous state at time t-1
...
def savedata(x, t, z):
    """
    the function is responsable of saving the acquired data in 2 csv files:
    1) "data_position.csv" -> the estimated states (x,y,theta) and the sensor measurements (x,y,theta) over the time;
    2) "plotdata.csv" -> the estimate states (x,y) and the sensor measurements (x,y) in order to plot the path in X-Y
    plane.
    """
    error_x.append(x[0])
    error_y.append(x[1])
...
def EKFmain(session, init_position, realMeasurement):
    """
    Main function.
    INPUT:
    1) 'session' to access the robot velocities;
    2) 'init_position'
    3) realMeasurement: the queue fro getting the sensor measurement.
    """
    ...
    while True:
        _time += deltat
        # -- loop -- #
        x, p, z = EKF(session, realMeasurement, x_t_minus_1, p_t_minus_1, deltat)
        ...
        # send data
        send_str = str(x[0][0]) + "," + str(x[1][0]) + "," + str(x[2][0])
        s.send(send_str)
        time.sleep(1)
    s.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # -- EKF thread -- #
        EKFThread = threading.Thread(target=EKFmain, args=(session, init_position, robotPose_w,))
        EKFThread.start()
